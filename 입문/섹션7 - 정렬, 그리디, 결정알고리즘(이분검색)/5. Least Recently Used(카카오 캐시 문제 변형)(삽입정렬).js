...
위 상태에서 만약 새로운 작업인 5번 작업을 CPU가 사용한다면
...
function solution(size, arr){
  ...
  arr.forEach(x => {
    ...
    // 캐시배열 순회하며 현재 arr의 요소 index를 찾음
    for(let i=0; i<size; i++) if(x===answer[i]) pos=i;

    if(pos===-1){
      // *** arr 요소가 없는 경우
      // 캐시배열을 맨 뒤부터 순회 (4, 3, 2, 1) 하며 앞에 요소들을 하나씩 땡긴다.
      ...
      // *** arr 요소가 있는 경우
      // 찾은 요소의 index부터 앞에 요소들을 하나씩 땡긴다.
      for(let i=pos; i>=1; i--){
        answer[i]=answer[i-1];
      }
    } 

    // *** arr 요소가 있건 없건 맨 앞에 현재 요소 추가
    answer[0]=x;  
  });
...
function solution(size, arr){
  ...
  arr.forEach(x => {
    ...
    if(pos===-1){
      // arr 요소가 없는 경우 맨 앞에 추가하고 길이가 길면 빼기
      answer.unshift(x);
      if(answer.length>size) answer.pop();
    }
    else{
      // arr 요소가 있으면 해당 요소 빼고 맨 앞에 추가
      answer.splice(pos, 1);
      answer.unshift(x);
    } 
